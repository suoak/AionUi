...
// Ship the pinned MCP package with WorkMate. Runtime downloads made the first connection
// exceed the UI's timeout and made the built-in feature unavailable offline.
const mcpEntryPath = resolveBundledChromeDevtoolsMcpPath(__dirname);

if (!existsSync(mcpEntryPath)) {
  logDiagnostic(`Bundled chrome-devtools-mcp entry is missing: ${mcpEntryPath}`);
  process.exit(1);
}

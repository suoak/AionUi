...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.CSBU_WORKMATE_IMG_PLATFORM;
  const base_url = process.env.CSBU_WORKMATE_IMG_BASE_URL;
  const api_key = process.env.CSBU_WORKMATE_IMG_API_KEY;
  const model = process.env.CSBU_WORKMATE_IMG_MODEL;
...
async function main() {
  ...
  server.tool(
    'csbu_workmate_image_generation',
...
IMPORTANT: When user provides multiple images, ALWAYS pass ALL images to the image_uris parameter as an array.`,
    ...
    async ({ prompt, image_uris }) => {
      ...
      const proxy = process.env.CSBU_WORKMATE_IMG_PROXY || undefined;

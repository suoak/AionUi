 ...
 * 用户看不见的 Chrome）。
 *
 * CSBU_WORKMATE_CDP_PORT 现在只当开关用（"0"/"false" 关闭，其它非空值开启）。它的数值不再
 ...
 *
 * CSBU_WORKMATE_CDP_PORT now acts purely as a switch ("0"/"false" disables, any other non-empty
...
function shouldEnableCdp(config: CdpConfig): boolean {
  const envVal = process.env.CSBU_WORKMATE_CDP_PORT;

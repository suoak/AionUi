...
class AutoUpdaterService extends EventEmitter {
  ...
  private moveCwdOutOfInstallDirForWindowsHandoff(): void {
    ...
    try {
      const safeCwd = path.join(app.getPath('temp'), 'csbu-workmate-updater-cwd');
   ...
   * while the downloaded archive contains the packaged app (com.csbu.workmate). Squirrel.Mac

...
// Every language CSBU WorkMate ships that Arco publishes a locale for. Arco has no
  ...
  void import('@sentry/electron/renderer')
    .then((Sentry) => {
      Sentry.withScope((scope) => {
        scope.setTag('csbu-workmate.installation_integrity', event.failure_kind ?? 'unknown');
        scope.setTag('csbu-workmate.runtime_resource', event.resource);
        scope.setTag('csbu-workmate.runtime_resource_id', event.resource_id ?? '');
        scope.setTag('csbu-workmate.runtime_scope', event.scope.kind);

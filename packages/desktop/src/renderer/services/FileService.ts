...
import { CSBU_WORKMATE_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理CSBU WorkMate时间戳后缀，返回原始文件名
export function cleanCsbuWorkMateTimestamp(file_name: string): string {
  return file_name.replace(CSBU_WORKMATE_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(file_path: string): string {
  const file_name = file_path.split(/[\\/]/).pop() || '';
  return cleanCsbuWorkMateTimestamp(file_name);
}
...
class FileServiceClass {
   ...
   * directory (`temp_dir/csbu-workmate/...`). Passing the raw device path (the old

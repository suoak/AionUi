...
const ENABLED_CONVERSATION_KEY = 'csbu-workmate:e2e-message-stream-conversation-id';
...
declare global {
  interface Window {
    __CSBU_WORKMATE_E2E_MESSAGE_STREAM__?: StreamRegistry;
  }
}
...
const AcpE2EStreamInjector: React.FC<{ conversationId: string }> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const registry = (window.__CSBU_WORKMATE_E2E_MESSAGE_STREAM__ ??= { controllers: {} });
    ...
    return () => {
      if (window.__CSBU_WORKMATE_E2E_MESSAGE_STREAM__) {
        delete window.__CSBU_WORKMATE_E2E_MESSAGE_STREAM__.controllers[conversationId];
      }
    };

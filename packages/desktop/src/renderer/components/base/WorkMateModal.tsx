...
/** WorkMateModal 组件 Props */
export interface WorkMateModalProps extends Omit<ModalProps, 'title' | 'footer'> {
...
const STD_BODY_LAYOUT_CLASS = 'csbu-workmate-modal-std-body min-h-0 flex-1 overflow-y-auto';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <WorkMateModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </WorkMateModal>
 *
 * // 预设尺寸 / Preset size
 * <WorkMateModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </WorkMateModal>
 *
...
const WorkMateModal: React.FC<WorkMateModalProps> = ({
  children,
  variant,
  size,
  header,
  ...
  const renderFooter = () => {
    ...
    if (footerConfig.render) {
      ...
      const footerClassName = classNames(
        ...
        isStandard && 'csbu-workmate-modal-std-footer',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={classNames('csbu-workmate-modal', isStandard && 'csbu-workmate-modal-standard', className)}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div
        className={classNames('csbu-workmate-modal-wrapper', isStandard && 'flex flex-col min-h-0')}
        style={{ borderRadius: borderRadiusVal }}
      >
        {renderHeader()}
        <div
          className={classNames(
            'csbu-workmate-modal-body-content',
...
WorkMateModal.displayName = 'WorkMateModal';

export default WorkMateModal;

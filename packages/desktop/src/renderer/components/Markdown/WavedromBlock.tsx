...
const MAX_WAVE_SOURCE_LENGTH = 200_000;
const MAX_WAVE_NODES = 5_000;

const isWithinComplexityLimit = (value: unknown): boolean => {
  let visited = 0;
  const stack: unknown[] = [value];
  while (stack.length > 0) {
    const current = stack.pop();
    visited += 1;
    if (visited > MAX_WAVE_NODES) return false;

...
describe('build-with-builder', () => {
  it('rejects skip-vite when renderer output is only a source html shell', () => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), 'csbu-workmate-build-skip-vite-test-'));
...
`,
  ...
  it('releases the NSIS output directory before any update repair or uninstall work', () => {
    ...
    const preInit = script.match(/!macro CSBU_WORKMATE_INSTALLER_PREINIT([\s\S]*?)!macroend/)?.[1];
    const releaseMacro = script.match(/!macro CSBU_WORKMATE_RELEASE_INSTALL_DIR_OUTDIR([\s\S]*?)!macroend/)?.[1];
    ...
    expect(preInit).toContain('!insertmacro CSBU_WORKMATE_RELEASE_INSTALL_DIR_OUTDIR');
    expect(preInit!.indexOf('CSBU_WORKMATE_RELEASE_INSTALL_DIR_OUTDIR')).toBeLessThan(
      preInit!.indexOf('CSBU_WORKMATE_SESSION_BEGIN')
    );
  });
  ...
  it('uses install-directory ownership checks in the shared Windows NSIS include', () => {
    ...
    expect(script).not.toContain("Name -ieq '${CSBU_WORKMATE_APP_EXECUTABLE_FILENAME}'");
  });
  ...
  it('records installer self-lock diagnostics when Restart Manager finds no locking process', () => {
    ...
    const captureMacro = script.match(/!macro CSBU_WORKMATE_CAPTURE_FAILED_PATH_LOCKERS[\s\S]*?!macroend/)?.[0];

    expect(script).toContain('csbu-workmate-query-lockers.ps1');
    expect(captureMacro).toContain('CSBU_WORKMATE_QUERY_LOCKERS');
    expect(captureMacro).not.toContain('CSBU_WORKMATE_QUERY_LOCKERS_INLINE_LEGACY');
    ...
    expect(queryScript).toContain('currentOutDir');
    expect(queryScript).toContain("name = 'CSBU WorkMate installer'");
  });
  ...
  it('continues with the bundled uninstaller when installed-uninstaller repair remains locked', () => {
    ...
    const retryFailureBranch = script.match(
      /\$\{If\} \$\{Errors\}\s+([\s\S]*?)\$\{Else\}\s+!insertmacro CSBU_WORKMATE_LOG_UNINSTALLER_REPAIR "after-copy-retry"/
    )?.[1];

    expect(retryFailureBranch).toBeTruthy();
    expect(retryFailureBranch).toContain('copy-failed-using-bundled');
    expect(retryFailureBranch).toContain('$CsbuWorkMateBundledUninstaller');
    expect(retryFailureBranch).not.toContain('MessageBox');
    expect(retryFailureBranch).not.toContain('CSBU_WORKMATE_MSG_UNINSTALLER_LOCKED');
  ...
  it('keeps coded Windows installer failures on the unified reportable failure path', () => {
    ...
    for (const file of files) {
      ...
      source.split(/\r?\n/).forEach((line, index) => {
        if (line.includes('!macro CSBU_WORKMATE_FAIL ')) {
          offenders.push(`${file}:${index + 1}: defines non-reportable coded failure macro`);
        }
        if (line.includes('!insertmacro CSBU_WORKMATE_FAIL ')) {
  ...
  it('routes app-cannot-be-closed cancellation through E1003 instead of quitting silently', () => {
    ...
    const cannotCloseBranch = script.match(
      /CSBU_WORKMATE_MSG_CLOSE_OR_REMOVE_PREVIOUS_ZH[\s\S]*?IDRETRY csbu_workmate_wait_for_close([\s\S]*?)\$\{Else\}/
    )?.[1];

    expect(cannotCloseBranch).toBeTruthy();
    expect(cannotCloseBranch).toContain('CSBU_WORKMATE_E_INSTALL_DIR_REMOVE_OR_LOCKED');
    expect(cannotCloseBranch).toContain('CSBU_WORKMATE_FAIL_REPORTABLE_BILINGUAL_DIAGNOSTICS');
  ...
  it.each([
    ...
    const tempDir = mkdtempSync(join(tmpdir(), 'csbu-workmate-build-test-'));
    const hookPath = join(tempDir, 'hook.cjs');
...
`,
    ...
    try {
      ...
      const result = spawnSync(process.execPath, ['scripts/build-with-builder.js', ...args], {
        ...
        env: {
          ...process.env,
          CSBU_WORKMATE_PREPARE_CALLS_FILE: callsPath,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, `--require=${hookPath}`].filter(Boolean).join(' '),
        },
      });
      ...
      expect(readFileSync(resolve(repoRoot, 'resources/windows/support/_sentry-dsn.generated.nsh'), 'utf8')).toBe(
        '!define CSBU_WORKMATE_SENTRY_DSN ""\n'
      );

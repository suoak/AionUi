...
describe('useManagedAgents', () => {
  ...
  it('subscribes to the management SWR key with the managed fetcher', () => {
    vi.mocked(useSWR).mockReturnValue({ data: [], error: null, isLoading: false } as never);
  ...
  it('exposes the agents returned by SWR', () => {
    ...
    vi.mocked(useSWR).mockReturnValue({ data: agents, error: null, isLoading: false } as never);
  ...
  it('falls back to an empty list when SWR has no data yet', () => {
    vi.mocked(useSWR).mockReturnValue({ data: undefined, error: null, isLoading: true } as never);
  ...
  it('getManagedAgents fetches the management catalog without invalidating the detected cache', async () => {
    ...
    vi.mocked(fetchManagedAgents).mockResolvedValue(managedAgents as never);

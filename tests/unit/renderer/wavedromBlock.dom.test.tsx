...
describe('WavedromBlock', () => {
  ...
  it('does not render oversized WaveJSON input', async () => {
    const oversized = `{ signal: [], note: '${'x'.repeat(200_001)}' }`;
    render(<WavedromBlock code={oversized} />);
    expect(await screen.findByTestId('wavedrom-source')).toBeInTheDocument();
    expect(renderAnyMock).not.toHaveBeenCalled();
  });

  it('does not render excessively complex WaveJSON input', async () => {
    const complex = JSON.stringify({ signal: Array.from({ length: 3_000 }, (_, index) => ({ name: `${index}` })) });
    render(<WavedromBlock code={complex} />);
    expect(await screen.findByTestId('wavedrom-source')).toBeInTheDocument();
    expect(renderAnyMock).not.toHaveBeenCalled();
  });
